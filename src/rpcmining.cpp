...
//    Litecoin and modified for SToin.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated SToin network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for SToin
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the SToin network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "SToin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "SToin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SToin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SToin is downloading blocks...");
